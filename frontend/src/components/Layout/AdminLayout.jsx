...
import { LayoutDashboard, Users, CalendarDays, BarChart3, Settings, LogOut, Trophy, Menu, X, TicketPercent, MessageSquare } from 'lucide-react';
...
const AdminLayout = ({ children }) => {
    ...
    const menuItems = [
        ...
        { name: 'Manage Bookings', path: '/admin/bookings', icon: <CalendarDays size={16} /> },
        { name: 'Manage Fields', path: '/admin/fields', icon: <Trophy size={16} /> }, 
        { name: 'Manage Discounts', path: '/admin/discounts', icon: <TicketPercent size={16} /> }, 
        { name: 'User Reviews', path: '/admin/reviews', icon: <MessageSquare size={16} /> }, 
    ];

    const handleLogout = () => {
        localStorage.clear();
        alert("Keluar dari sistem admin sukses.");
        navigate('/login');
    };
    ...
    return (
        <div className="min-h-screen bg-[#f8f9ff] text-[#0b1c30] font-sans flex flex-col md:flex-row pt-4">
            ...
            {/* UNIFIED SIDEBAR NAVIGATION (Kiri Permanen - Meniru image_4c8e3f.png) */}
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-xl border border-slate-100">
                        ...
                        <div className="overflow-hidden">
                            <h3 className="font-bold text-xs text-slate-800 truncate">{user.nama || 'Admin'}</h3>
                            <p className="text-[9px] font-bold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded uppercase tracking-wider mt-0.5">Premium Admin</p>
                        </div>
                    </div>
            ...
            <div className="flex items-center justify-between md:hidden bg-white p-4 mx-4 rounded-xl border border-slate-200/50 shadow-sm mb-2">
                ...
                <span className="font-mono font-black text-xs text-[#004ac6] uppercase tracking-wider">Admin Panel</span>
            </div>
